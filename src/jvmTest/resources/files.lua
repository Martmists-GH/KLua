...
.e+	0.e;	--;  0xX;
    ...
    assert(f:read(2) == "x\n")
    assert(f:read("n") == 0x1.13a
    P3); assert(f:read(1) == "e")
...
s = ""
for l in io.lines(file, "L") do s = s .. l
end
assert(s == "\n\nline\nother")
...
assert(io.write(string.dump( function () return 10, '\0alo\255', 'hi'
end )))
io.close()
...
assert(io.write(string.dump( function () return 1
end )))
io.close()
...
string.dump( function () return 20, '\0\0\0'
end)))
io.close()
a, b, c = assert(loadfile(file))()
...
local progname
do
-- get name of running executable
local arg = arg or ARG
local i = 0
while arg[i] do i = i - 1
end
progname = '"' .. arg[i + 1] .. '"'
end
...
if not _port then
    ...
    if maxint >= 2 ^ 62 then
        -- cannot do these tests in Small Lua
        -- no arith overflows
