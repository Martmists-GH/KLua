...
assert(0e12 == 0 and .0 == 0 and 0. == 0 and .2
e2 == 20 and 2.
E - 1 == 0.2)
...
-- try 'i' as integer and as float
for _, tj in pairs{0, 0.0} do
-- try 'j' as integer and as float
local x = i + ti
local y = j + tj
...
assert(eq((-3)^-3, 1 / (-3)^3))
for i = -3, 3 do
-- variables avoid constant folding
for j = -3, 3 do
...
-- floats can express all integers with full accuracy
...
local function f2i (x) return x | x
end
...
if not _port then
local function anan (x) assert(isNaN(x))
