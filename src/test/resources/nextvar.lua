...
do
    -- rehash moving elements from array to hash
    local a = {}
    for i = 1, 100 do
        a[i] = i
    end
...
if not T then
    ...
    for _, sa in ipairs(sizes) do
        -- 'sa' is size of the array part
        local arr = { "return {" }
        for i = 1, sa do
            arr[1 + i] = "1,"
        end    -- build array part
        for _, sh in ipairs(sizes) do
            -- 'sh' is size of the hash part
            for j = 1, sh do
                -- build hash part
    ...
    for i = 1, 64 do
        a[i] = true
    end    -- make its array size 64
    for i = 1, 64 do
        a[i] = nil
    end     -- erase all elements
    assert(T.querytab(a) == 64)    -- array part has 64 elements
    a[32] = true;
    a[48] = true;    -- binary search will find these ones
...
do
    -- testing 'next' with all kinds of keys
    local a = {
        [1] = 1, -- integer
        [1.1] = 2, -- float
        ['x'] = 3, -- short string
...
do
    -- testing table library with metamethods
...
do
    -- testing overflow in table.insert (must wrap-around)
...
if not T then
    ...
    debug.setmetatable(u2, { __newindex = function(_, k, v)
        tab2[k] = v
    end })
...
do
    -- testing other strange cases for numeric 'for'
