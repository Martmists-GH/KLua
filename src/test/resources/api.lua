...
do
    -- test returning more results than fit in the caller stack
    local a = {}
    for i = 1, 1000 do
        a[i] = true
    end ;
...
do
    -- testing lessthan and lessequal with metamethods
...
-- testing length
local t = setmetatable({ x = 20 }, { __len = function(t)
    return t.x
end })
a, b, c = T.testC([[
   len 2;
...
a = setmetatable({ x = "u" }, { __concat = function(a, b)
    return a.x .. '.' .. b.x
end })
x, y = T.testC([[
  pushnum 5
...
do
    -- using the table itself as index
    local a = {}
    a[a] = 10
...
--
do
    -- testing getfield/setfield with long keys
...
do
    ...
    for i = 1, 20, 2 do
        -- mark half of them to raise errors during GC
    ...
    for i = 2, 20, 2 do
        -- mark the other half to count and to create more garbage
...
do
    -- testing failing in 'lua_checkstack'
...
do
    -- closing state with no extra memory
    local L = T.newstate()
    T.alloccount(0)
    T.closestate(L)
    T.alloccount()
...
do
    -- garbage collection with no extra memory
    local L = T.newstate()
    T.loadlib(L)
