...
import com.martmists.klua.ext.toDegrees
import com.martmists.klua.ext.toRadians
...
fun TTable.insertMath() {
    ...
    this["deg"] = TFunction { args ->
        val num = args.argument(0, LuaType.NUMBER) as TNumber<*>
        return_(TDouble(num.value.toDouble().toDegrees()))
    }
    ...
    this["rad"] = TFunction { args ->
        val num = args.argument(0, LuaType.NUMBER) as TNumber<*>
        return_(TDouble(num.value.toDouble().toRadians()))
    }
    val randomInstance = Xorshiro256(0UL, 0UL, 0UL, 0UL).also {
        val seed1 = (Clock.System.now().toEpochMilliseconds() / 1000L).toULong()
        val seed2 = it.hashCode().toULong()
    ...
    this["randomseed"] = TFunction { args ->
        ...
        if (x.value == 0L && y.value == 0L) {
            val seed1 = (Clock.System.now().toEpochMilliseconds() / 1000L)
            val seed2 = randomInstance.hashCode()
    ...
    this["tointeger"] = TFunction { args ->
        val res = when (val num = args.argument(0)) {
            is TDouble -> {
                if (num.isInteger()) {
