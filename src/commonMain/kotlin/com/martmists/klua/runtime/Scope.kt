                ...
                outer@for (target in node.targets) {
                    ...
                    if (target.owner == null) {
                        if (node.local) {
                            ...
                            inner@for (t in scopeQueue) {
                                if (key in t.env.value) {
                                    collectAsLuaScope {
                                        t.env.luaNewIndex(key, value)
                                    }
                                    continue@outer
                                }
                            }

                            // Variable not found before, assign to current scope
                            collectAsLuaScope {
                                env.luaNewIndex(key, value)
                            }
                        }
                    } else {
